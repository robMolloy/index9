import { innerJoinArrayOfObjects } from '../..'
...
test('join contacts to customers ', () => {
  const customerContacts = innerJoinArrayOfObjects(contacts)
    .to(customers,)
    .where('cus_id')
    .matches('id')
...
test('join prefixedContacts to customers ', () => {
  const customerContacts = innerJoinArrayOfObjects(contacts, 'con_')
    .to(customers)
    .where('cus_id')
    .matches('id')
